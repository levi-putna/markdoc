...
- **`QUICKLOOK_SIGN_IDENTITY` and `QUICKLOOK_TEAM_ID`**: consumed separately by `macos/scripts/build-quicklook.sh` (not by electron-builder). If unset, the Quick Look extension silently falls back to ad-hoc signing (`CODE_SIGN_IDENTITY=-`) — the build succeeds locally, but **notarization fails** at the very end, after the Apple round-trip, with errors about a missing Developer ID signature, no secure timestamp, and a lingering `get-task-allow` entitlement. Verify these are set *before* running `yarn release`, not after hitting the failure. `QUICKLOOK_SIGN_IDENTITY` must exactly match a `security find-identity -v -p codesigning` entry (e.g. `"Developer ID Application: Name (TEAMID)"`) and must be quoted in `.env` (it contains spaces/colons/parens, which breaks unquoted `source .env`).
- **First use of a newly-created signing identity**: the very first `codesign` call against a fresh Developer ID private key triggers a macOS Keychain permission dialog ("codesign wants to access key... Always Allow / Deny") that blocks the build until a human clicks it — this can't be observed or dismissed from the terminal. If a `build:quicklook` or `release` run hangs with no new output, check for a `SecurityAgent` process (`ps aux | grep SecurityAgent`) and ask the user to check their screen.
